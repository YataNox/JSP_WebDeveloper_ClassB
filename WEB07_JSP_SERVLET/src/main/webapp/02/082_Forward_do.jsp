...
<%
	...
	// request 객체에는 Attribute라는 저장소가 있습니다.
	// 전달한 데이터를 Attribute에다가 각 자료의 이름을 같이 저장하고, forward(이동)하면,
	// 목적지에 해당 데이터가 같이 이동합니다.
	
	// Attribute라는 저장소에 name이라는
	// 이름으로 "홍길동"을 저장해두고 083_ForwardResult.jsp로 이동(Forward)합니다.
	...
	// 현재 페이지의 request 객체의 수명은 forward로 전달될 다음 페이지까지 입니다.
	// 보통 파라미터를 통해서 정보를 전달하고 request.getParameter()를 사용하여 전달된 값을
	// 추출하여 사용하지만, 위의 RequestDispatcher는 전달인수 대신 request 내부의
	// Attribute를 사용하여 전달 인자를 저장하고, 수명을 다할지 모를 이전 페이지의 request와
	// response를 전달 인수로 전달하여 forward라는 명령으로 페이지를 이동한 다음..
