...
<html>
	...
	<body>
		...
		// eDay는 현재 날짜의 다음달 1일로 세팅 후 1일을 빼서 이번 달 마지막날로 세팅
		eDay.set(sYear, sMonth+1, 1); // 다음 달 1일
		eDay.add(Calendar.DATE, -1); // 이번달 말일
		
		// 출력하려는 월의 1일자가 무슨 요일인지 정수 값으로 계산 1:일요일, 2:월요일
		...
		<table width="560" align="left" cellspacing="1" bgcolor="black">
			...
			<tr bgcolor="white" height="50">
				<%for(i = 1; i<START_WEEK; i++){ %>
						<td>&nbsp;</td><!-- 첫 칸부터 시작 날짜의 요일바로 전열까지 빈 칸 표시 -->
				<%} %>
			...
			<tr bgcolor="white" height="60">
				<%for(k = i; k<=eDay.get(Calendar.DATE); k++){ %>
						...
						<!-- 일요일부터 날짜까 출력되고 있기 때문에 1부터 시작한 cnt 변수로 카운트해서
						줄바꿈을 실행합니다.  -->
						<%
						cnt++;
						if(cnt%7 == 0){
